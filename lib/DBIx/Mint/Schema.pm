...
Receives a list of attributes of the given class which are not stored in the database.
...
If present, this parameter defines the name of a method which is inserted into the 'one' class which allows it to insert related records into the 'many' class. It expects hash references as input. Note that they should have the same keys in order to profit from a prepared insert statement.
